...
constexpr uint32_t COL_BACKGROUND = 0xff9f2b68;
...
int main()
{
	...
	constexpr double step1 = 0.3, step2 = 0.01;
	constexpr double  masshtab = 30.0;
	constexpr double Oxy = 20.0;
	constexpr double Oz = 300.0;
	std::vector<int> horizontLine(IMG_WIDTH);
	for(auto && value : horizontLine)
		value = IMG_HEIGHT;
	for (double i = masshtab; i > -masshtab; i -= step1) {
		for (double j = masshtab; j > -masshtab; j -= step2) {
			double z = (my_evil_function(i, j));
			int x = int( IMG_WIDTH / 2 + Oxy * cos( Pi/6.0 )*(i - j) );
			int y = int( IMG_HEIGHT / 2 + Oxy * sin( Pi/6.0 )*(i  + j) - Oz * z);
	...
	for(auto && value : horizontLine)
		value = IMG_HEIGHT;
	for (double j = masshtab; j > -masshtab; j -= step1) {
		for (double i = masshtab; i > -masshtab; i -= step2) {
